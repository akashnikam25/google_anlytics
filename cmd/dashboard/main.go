...
func main() {
	...
	for {
		...
		case "p":
			renderPageViews()
		case "b":
			what = gotracker.QueryBrowsers
			renderPie("Browsers")
		case "o":
			what = gotracker.QueryOSes
			renderPie("OSes")
		case "c":
			what = gotracker.QueryCountry
			renderPie("Countries")
		case "r":
			what = gotracker.QueryReferrerHost
			...
			renderTable()
		case "t":
			if what == gotracker.QueryPageViews {
				metrics, err := getMetric(gotracker.QueryPageViewList)
				if err != nil {
					log.Fatal(err)
				}
		...
		case "z":
			...
			end = int64(gotracker.TimeToInt(time.Now()))
			days := 30
			if dateRangeMode == 1 {
				days = 90
			...
			start = int64(gotracker.TimeToInt(time.Now().Add(-24 * time.Duration(days) * time.Hour)))
...
func renderPageViews() {
	what = gotracker.QueryPageViews

	ui.Clear()

	metrics, err := getMetric(gotracker.QueryPageViews)
	if err != nil {
		log.Fatal(err)
	}
